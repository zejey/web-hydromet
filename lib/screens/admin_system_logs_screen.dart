...
class _AdminSystemLogsScreenState extends State<AdminSystemLogsScreen> {
  ...
  final List<Map<String, dynamic>> _adminSystemLogs = [
    {
  ...
  void initState() {
    super.initState();
    _filteredLogs = List.from(_adminSystemLogs);
    // Debug print to check if logs are loaded
    // ignore: avoid_print
    print(
      'AdminSystemLogsScreen: _adminSystemLogs.length = \\${_adminSystemLogs.length}, _filteredLogs.length = \\${_filteredLogs.length}',
    );
  }

  void _applyFilters() {
    List<Map<String, dynamic>> filtered = List.from(_adminSystemLogs);
    ...
    setState(() {
      ...
      // ignore: avoid_print
      print(
        'APPLY FILTERS: _adminSystemLogs.length = \\${_adminSystemLogs.length}, _filteredLogs.length = \\${_filteredLogs.length}',
      );
    });
  }
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Text(
                      '${_filteredLogs.length} of ${_adminSystemLogs.length} logs',
